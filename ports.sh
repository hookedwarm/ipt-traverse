...
chain_arg="TCP_PORTS_"
for port in $(cat ${var_dir}$tcp_src_ports_list); do
	./ipt_traverse.sh -t "$1" -A "$chain_arg" -r "-p tcp --sport $port" -j "RETURN"
done
for port in $(cat ${var_dir}$tcp_dst_ports_list); do
	./ipt_traverse.sh -t "$2" -A "$chain_arg" -r "-p tcp --dport $port" -j "RETURN"
done
chain_arg="UDP_PORTS_"
for port in $(cat ${var_dir}$udp_src_ports_list); do
	./ipt_traverse.sh -t "$1" -A "$chain_arg" -r "-p udp --sport $port" -j "RETURN"
done
for port in $(cat ${var_dir}$udp_dst_ports_list); do
